...
for i in range(0, item_row):
    for j in range(0, item_col):
        if i == j:
            ...
            item_similarity[i, j] = 1-item_similarity[i, j]
...
for i in range(0, user_row):
    for j in range(0, user_col):
        if i == j:
            ...
            user_similarity[i, j] = 1-user_similarity[i, j]
...
for i in range(0, item_row):
    ind = np.argpartition(item_similarity[i, :], -30)[-30:]
    for j in range(0, item_col):
        if j not in ind:
            item_similarity[i, j] = 0

for i in range(0, user_row):
    ind = np.argpartition(user_similarity[i, :], -30)[-30:]
    for j in range(0, user_col):
        if j not in ind:
            user_similarity[i, j] = 0
